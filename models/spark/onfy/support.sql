...
last_tag_ids AS (
    SELECT 
        mart.onfy_babylone_events.payload.ticketId AS ticket_id,
        from_utc_timestamp(MAX(mart.onfy_babylone_events.event_ts_utc), 'Europe/Berlin') as event_ts_cet,
        MAX_BY(mart.onfy_babylone_events.payload.tagIds, mart.onfy_babylone_events.event_ts_utc) AS tag_ids
    FROM 
        mart.onfy_babylone_events
    WHERE 
        mart.onfy_babylone_events.type = 'ticketChange'
        AND array_size(mart.onfy_babylone_events.payload.tagIds) > 0
    GROUP BY
...
tag_names AS (
    SELECT DISTINCT
        tag_ids.*,
        mongo.babylone_onfy_tags_daily_snapshot.name as tag_name
    FROM 
        tag_ids
        LEFT JOIN mongo.babylone_onfy_tags_daily_snapshot
            ON tag_ids.tag_id = mongo.babylone_onfy_tags_daily_snapshot._id
),
...
ticket_entry AS (
    SELECT DISTINCT
        ...
        ,mart.onfy_babylone_events.payload.ticketId AS ticket_id
        ,from_utc_timestamp(mart.onfy_babylone_events.event_ts_utc, 'Europe/Berlin') as event_ts_cet
        --,mart.onfy_babylone_events.payload.changedById
        --,mart.onfy_babylone_events.payload.stateOwner
        ,mart.onfy_babylone_events.payload.authorType
    FROM 
        mart.onfy_babylone_events
    WHERE 
        mart.onfy_babylone_events.type = 'ticketEntryAdd'
        AND mart.onfy_babylone_events.payload.entryType = 'message'
),
...
ticket_resolved AS (
    ...
    WHERE 
        mart.onfy_babylone_events.type = 'ticketChange'
        and mart.onfy_babylone_events.payload.stateOwner in ('Resolved', 'Rejected')
),
...
ticket_create AS (
    ...
    FROM
        ...
        LEFT JOIN ticket_entry as customer_message
            ON customer_message.ticket_id = ticket_create_predata.ticket_id
            AND customer_message.authorType = 'customer'
        LEFT JOIN ticket_entry as agent_message
            ON agent_message.ticket_id = ticket_create_predata.ticket_id
...
SELECT DISTINCT
    ...
    (UNIX_SECONDS(TIMESTAMP(ticket_create.first_agent_message_cet))
            - UNIX_SECONDS(TIMESTAMP(ticket_created_cet))) / 3600 as time_to_first_reply,
    ticket_create.ticket_resolved_cet,
    (UNIX_SECONDS(TIMESTAMP(ticket_create.ticket_resolved_cet))
            - UNIX_SECONDS(TIMESTAMP(ticket_created_cet))) / 3600 as time_to_resolution,
...
FROM
    ...
    LEFT JOIN tag_names as tag_complaint_damage
        ON tag_complaint_damage.ticket_id = ticket_create.ticket_id
        AND tag_complaint_damage.tag_name = 'complaint_damage'
    LEFT JOIN tag_names as tag_complaint_pharmaceutical_reasons
        ON tag_complaint_pharmaceutical_reasons.ticket_id = ticket_create.ticket_id
    ...
    LEFT JOIN tag_names as tag_pre_sales_pharmaceutical_reasons
        ON tag_pre_sales_pharmaceutical_reasons.ticket_id = ticket_create.ticket_id
