{{
  config(
    materialized='table',
    alias='jms_merchant_orders_full',
    file_format='parquet',
    schema='category_management',
    clustered_by=['marketplace_name'],
    partition_by=['order_month_utc'],
    meta = {
...
INNER JOIN {{ ref('gold_merchant_categories') }} AS gmc ON gp.merchant_category_id = gmc.merchant_category_id