...
SELECT
    ...
    TO_DATE(mo.created_time_utc) AS created_date,
...
LEFT JOIN cr_data AS cr
    ON
        mo.created_time_utc BETWEEN cr.effective_date AND cr.next_effective_date
        AND mo.money_info.merchant_currency = cr.currency_code
WHERE
    source.kind = 'jms'
    AND TO_DATE(mo.created_time_utc) >= '2023-02-16' -- date of first JMS order
