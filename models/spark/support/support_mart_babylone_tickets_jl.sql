...
messages_last_replies AS (
    ...
    FROM
        first_entries AS t
    INNER JOIN closing_logistics AS a ON t.ticket_id = a.ticket_id
    GROUP BY
        1,
        2,
        3
...
all_queues AS (
    WITH t AS (
        ...
        FROM
            mart.logistics_babylone_events AS t
        INNER JOIN {{ source('mongo', 'babylone_logistics_queues_daily_snapshot') }} AS a ON t.payload.stateQueueId = a._id
        WHERE
            t.`type` = 'ticketChange'
