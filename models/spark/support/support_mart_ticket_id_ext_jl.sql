...
first_queue AS (
    ...
    FROM mart.logistics_babylone_events AS t
    INNER JOIN {{ source('mongo', 'babylone_logistics_queues_daily_snapshot') }} AS a
...
all_tags AS (
    ...
    base AS (
        SELECT
            ...
            a.name AS tag
        FROM t AS t
        INNER JOIN {{ source('mongo', 'babylone_logistics_tags_daily_snapshot') }} AS a ON t.tag = a._id
    )
...
responses AS (
    WITH ranking AS (
        ...
        FROM mart.logistics_babylone_events AS t
        INNER JOIN mart.logistics_babylone_events AS a
            ON
...
LEFT JOIN {{ source('mongo', 'babylone_logistics_agents_daily_snapshot') }} AS y ON t.author_id = y._id
