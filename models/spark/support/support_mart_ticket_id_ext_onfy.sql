...
ttfr AS (
    SELECT
        ...
        mfr.partition_date,
        ((UNIX_SECONDS(TIMESTAMP(mfr.ts_first_replied)) - UNIX_SECONDS(TIMESTAMP(mfr.ts_created))) / 3600) AS ttfr
    FROM ticket_create_events AS tickets
    LEFT JOIN messages_first_replies AS mfr
...
first_queue AS (
    ...
    FROM {{ source('mart', 'onfy_babylone_events') }} AS t
    INNER JOIN {{ source('mongo', 'babylone_onfy_queues_daily_snapshot') }} AS a
...
all_tags AS (
    ...
    base AS (
        SELECT
            ...
            a.name AS tag
        FROM t AS t
        INNER JOIN {{ source('mongo', 'babylone_onfy_tags_daily_snapshot') }} AS a ON t.tag = a._id
    )
...
responses AS (
    WITH ranking AS (
        ...
        FROM {{ source('mart', 'onfy_babylone_events') }} AS t
        INNER JOIN {{ source('mart', 'onfy_babylone_events') }} AS a
            ON
...
LEFT JOIN {{ source('mongo', 'babylone_onfy_agents_daily_snapshot') }} AS y ON t.author_id = y._id
