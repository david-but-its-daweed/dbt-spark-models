...
messages_last_replies AS (
  ...
  FROM
    first_entries AS t
    JOIN closing_narwhal AS a ON t.ticket_id = a.ticket_id
  GROUP BY
    1,
    2,
    3
...
all_queues AS (
  WITH t AS (
    ...
    FROM
     mart.narwhal_babylone_events AS t
      JOIN {{ source('mongo', 'babylone_narwhal_queues_daily_snapshot') }} AS a ON t.payload.stateQueueId = a._id
    WHERE
      t.`type` = 'ticketChange'
...
all_tags AS (
  ...
  FROM
    t
    JOIN {{ source('mongo', 'babylone_narwhal_tags_daily_snapshot') }} AS a ON t.tag = a._id
),
all_markers AS (
  SELECT
    t.payload.ticketId AS ticket_id,
...
bot_result AS (
  ...
  FROM
    {{ ref('support_mart_ticket_id_ext_joompay') }}
