...
WITH orders AS (
    ...
    (
        ...
        WHERE
            (partition_date  < CURRENT_DATE()) AND
            (partition_date  >= CURRENT_DATE() - INTERVAL 240 day)
    )
),
...
sales_calendar AS (
    ...
    FROM (
        SELECT CASE
            ...
            ELSE LOWER(promo_title) END promo_title,
...
SELECT
    day AS partition_date_msk,
    CASE WHEN promo_start_date IS NOT NULL THEN CONCAT(CONCAT(promo_start_date,"-"), promo_end_date)
            ELSE "no_sales" END as sale_type,
    promo_start_date AS start_of_sale,
    promo_end_date AS end_of_sale,
    sale_period,
    SUM(gmv_initial) AS gmv_initial
FROM orders  AS o
LEFT JOIN sales_calendar AS a ON o.day >= promo_start_date AND o.day < promo_end_date
WHERE 1 = 1
{% if is_incremental() %}
    AND day >= date'{{ var("start_date_ymd") }}'
    AND day < date'{{ var("end_date_ymd") }}'
{% else %}
    AND day   >= date'2022-12-15'
{% endif %}
GROUP BY 1, 2, 3, 4, 5