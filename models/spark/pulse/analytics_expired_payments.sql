...
payments AS (
    ...
    FROM (
        SELECT
            ...
            MILLIS_TO_TS(payment.paidTimeMs) AS paid_time,
            TO_DATE(MILLIS_TO_TS(payment.paidTimeMs)) AS paid_date,
...
SELECT
    ...
    payment.package_price_ccy,
    payment.payment_created_time AS paid_time,
    TO_DATE(payment.payment_created_time) AS paid_date,
    payment.price,
    payment.currency,
    payment.promocode_id,
    payment.promocode,
    payment.discount_fixed,
    payment.discount_percentage,
    currency.rate,
    payment.status,
    payment.payment_number
FROM payments AS payment
INNER JOIN currency ON TO_DATE(payment.package_date) = currency.date
WHERE payment.payment_number != 0
