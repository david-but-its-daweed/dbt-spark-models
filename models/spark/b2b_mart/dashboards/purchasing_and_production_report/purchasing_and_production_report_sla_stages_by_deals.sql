...
small_batch_deals AS (
    SELECT
        ...
        -- флаги готовности по каждой стадии
        -- по Confirmation
        (
        ...
        MIN(CASE WHEN stage = '0.Assigned' THEN start_ts END) AS confirmation_start,
        MAX(CASE WHEN stage = '15.Waiting Payment' THEN end_ts END) AS confirmation_end,

        MIN(CASE WHEN stage = '2.Payment to Merchant' THEN start_ts END) AS chinaops_start,
        MAX(CASE WHEN stage = '5.Ready for Shipment' THEN end_ts END) AS chinaops_end,

        MIN(CASE WHEN stage = '5.Ready for Shipment' THEN start_ts END) AS shipped_start,
        MAX(CASE WHEN stage = '6.Shipped' THEN end_ts END) AS shipped_end
    FROM small_batch_deals_raw
    WHERE stage IN (
        '0.Assigned', '15.Waiting Payment',
        '2.Payment to Merchant', '5.Ready for Shipment', '6.Shipped'
    )
    GROUP BY 1,2
),
...
main AS (
    SELECT
        deal_friendly_id,
        is_small_batch,
        exploded_row.col.stage AS stage,
        'day' AS sla_granularity,
        exploded_row.col.sla_value AS sla_value,
        exploded_row.col.start_ts AS start_ts,
        CASE
            WHEN exploded_row.col.stage = '1.Confirmation' AND is_ready_confirmation THEN exploded_row.col.end_ts
            WHEN exploded_row.col.stage = '2.China Operations' AND is_ready_chinaops THEN exploded_row.col.end_ts
            WHEN exploded_row.col.stage = '3.Shipped' AND is_ready_shipped THEN exploded_row.col.end_ts
        END AS end_ts
    FROM small_batch_deals
    LATERAL VIEW EXPLODE(ARRAY(
        NAMED_STRUCT('stage', '1.Confirmation', 'sla_value', 4, 'start_ts', confirmation_start, 'end_ts', confirmation_end),
        NAMED_STRUCT('stage', '2.China Operations', 'sla_value', 14, 'start_ts', chinaops_start, 'end_ts', chinaops_end),
        NAMED_STRUCT('stage', '3.Shipped', 'sla_value', 7, 'start_ts', shipped_start, 'end_ts', shipped_end)
    )) exploded_row
),
