...
select
    ...
    avg(if(tracking_destination_country_time_utc is not null and (refund_type is null or refund_type != "not_delivered") or delivery_duration_tracking is not null, 1, 0)) share_data_exist, --заказы для которых сможем посчитать скорость
