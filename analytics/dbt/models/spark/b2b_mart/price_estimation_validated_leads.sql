...
users as (
    ...
    (select user_id, 
        ...
        date(min(case when validation_status = "validated" then event_ts_msk end) over (partition by user_id)) as min_validation_time
    from {{ ref('fact_user_change') }}
    ) a inner join not_jp_users n on a.user_id = n.user_id
...
orders as (
    ...
    inner join (select distinct order_id, interaction_id from {{ ref('fact_interactions') }} where rn = 1) i on o.order_id = i.order_id
    )
    )
