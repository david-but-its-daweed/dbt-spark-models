...
rfq as (
    SELECT 
        ...
        min(case when request_created_ts_msk is not null then response_created_ts_msk end) over (partition by order_id) as event_ts_msk,
        rfq_request_id,
        order_rfq_response_id,
        product_id,
        response_status,
    ...
    union all 
        ...
        min(case when request_sent_ts_msk is not null then response_created_ts_msk end) over (partition by order_id) as event_ts_msk,
        rfq_request_id,
        order_rfq_response_id,
        product_id,
        response_status,
    ...
    union all 
        ...
        min(case when response_created_ts_msk is not null then response_created_ts_msk end) over (partition by order_id, order_rfq_response_id) as event_ts_msk,
        rfq_request_id,
        order_rfq_response_id,
        product_id,
        response_status,
...
stg1 AS (
    SELECT
        ...
        COALESCE(o.sub_status, o.status) as sub_status,
        o.event_ts_msk,
        MAX(o.event_ts_msk) OVER (PARTITION BY o.order_id) AS event_ts,
        MAX(o.event_ts_msk) OVER (PARTITION BY o.order_id, order_rfq_response_id) AS event_product_ts,
