...
SELECT DISTINCT
    ...
    RANK() OVER (PARTITION BY order_id ORDER by rfq_response_ts_msk is null or order_rfq_response_id is null, rfq_response_ts_msk, order_rfq_response_id) as order_rn,
    RANK() OVER (PARTITION BY order_id, rfq_request_id ORDER BY rfq_response_ts_msk is null or order_rfq_response_id is null, rfq_response_ts_msk, order_rfq_response_id) as rfq_rn,
    RANK() OVER (PARTITION BY order_id, rfq_request_id, merchant_id  ORDER BY rfq_response_ts_msk is null or order_rfq_response_id is null, rfq_response_ts_msk, order_rfq_response_id) as rfq_merchant_rn,
