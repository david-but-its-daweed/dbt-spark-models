...
(
    SELECT
        ...
        TIMESTAMP(MIN(event_ts_msk)) OVER (PARTITION BY device_id, user.userId,  DATE(event_ts_msk))  AS event_ts_min_msk,
        TIMESTAMP(MAX(event_ts_msk)) OVER (PARTITION BY device_id, user.userId,  DATE(event_ts_msk))  AS event_ts_max_msk,
