...
(
    SELECT
        ...
        TIMESTAMP(MIN(device.osType)) OVER (PARTITION BY device_id, user.userId,  DATE(event_ts_msk))  AS event_ts_min_msk,
        TIMESTAMP(MAX(device.osType)) OVER (PARTITION BY device_id, user.userId,  DATE(event_ts_msk))  AS event_ts_max_msk,
