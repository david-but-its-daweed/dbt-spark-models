...
SELECT event_id,
    ...
    TIMESTAMP(payload.roleSet.roles.`owner`.actualisationTime.time) AS owner_ts_utc,
    payload.roleSet.roles.`owner`.moderatorId AS owner_moderator_id,
    TIMESTAMP(payload.roleSet.roles.`bizDev`.actualisationTime.time) AS biz_dev_ts_utc,
